...
_efl_gesture_recognizer_long_tap_efl_gesture_recognizer_create(Eo *obj EINA_UNUSED, Efl_Gesture_Recognizer_Long_Tap_Data *pd EINA_UNUSED,
                                                                Efl_Object *target EINA_UNUSED)
{
   return efl_add(EFL_GESTURE_LONG_TAP_CLASS, NULL);
                                                                   ...
                                                                   Efl_Gesture_Touch *event)
{
    ...
    switch (efl_gesture_touch_type_get(event))
      {
         case EFL_GESTURE_TOUCH_BEGIN:
           {
              efl_gesture_touch_start_point(event, &x, &y);
              efl_gesture_long_tap_position_set(gesture, EINA_POSITION2D(x, y));
